...
def main(args):
    annotation_file = os.path.join(args.input_dir, 'annotations/test2017.json')
    os.makedirs(os.path.join(args.input_dir, 'mask_annotations'), exist_ok=True)
    os.makedirs(os.path.join(args.input_dir, 'Binary_map_aug'), exist_ok=True)
    os.makedirs(os.path.join(args.input_dir, 'mask_JPEGImages'), exist_ok=True)
    ...
    for imgId in tqdm.tqdm(imgIds, ncols=100):
        ...
        if len(annIds) > 0:
            ...
            img_origin_path = os.path.join(args.input_dir, 'JPEGImages',  img['file_name'])
            img_output_path = os.path.join(args.input_dir, 'mask_JPEGImages', img['file_name'])
            seg_output_path = os.path.join(args.input_dir, 'mask_annotations', img['file_name'].replace('.jpg', '.png'))
...
def get_args():
    ...
    parser.add_argument("--input_dir", default="/home/zytx121/mmrotate/data/HRSID/", type=str,
